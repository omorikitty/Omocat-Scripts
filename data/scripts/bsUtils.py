...
class Background(bsGame.Actor):
    ...
    def __init__(self, fadeTime=500, startFaded=False, showLogo=False):
        ...
        with bs.Context(session):
            ...
            #             loop=False)
            # if showLogo:
            #     logoTexture = bs.getTexture('logoEaster')
...
def _runServer():
    ...
    if _gRunServerFirstRun:
        env = bs.getEnvironment()
        print ('\033[95m'+'BombSquad headless ' if bs.getEnvironment()['subplatform']
               == 'headless' else 'BombSquad ') + str(env['version'])+\
               ' ('+str(env['buildNumber'])+') entering server-mode '+\
               time.strftime('%c')+'\033[0m'
    ...
    if _gRunServerFirstRun:
        ...
        def accessCheckResponse(data):
            ...
            if data is None:
                ...
                if data['accessible']:
                    print '\033[96mUDP port', port, ('access check successful. Your '
                                             'server appears to be joinable '
                                             'from the internet.\033[0m')
                else:
                    print '\033[91mUDP port', port, (
                        ...
                        if not _gRunServerPlaylistFetch['sentRequest']:
                            ...
                            def onPlaylistFetchResponse(result):
                                ...
                                typeName = (
                                    ...
                                    'Free-for-All' else '??')
                                # print('\033[93m\''+'Playlist \'' + result['playlistName'] +
                                #       '\' (' + typeName +
                                #       ') downloaded; running...\''+'\033[0m')
                            ...
                            print(
                                '\033[94m'+'Requesting shared-playlist ' +
