...
class PowerupFactory(object):
    ...
    def __init__(self):
        ...
        self.texHealth = bs.getTexture("heart")
...
def getDefaultPowerupDistribution():
    if some.modded_powerups:
        ...
        return (('tripleBombs', 3), ('iceBombs', 0), ('punch', 0),
                ('impactBombs', 3), ('landMines', 2), ('stickyBombs', 3),
                ('shield', 0), ('health', 1), ('curse', 0))
...
class Powerup(bs.Actor):
    ...
    def __init__(self,
        ...
        if some.show_powerup_name:
            ...
            # def colori():
            #     if self._powText.exists():
            #         self._powText.color = (random.uniform(0.5, 3), random.uniform(0.5, 3), random.uniform(0.5,3))
            #     else:
            #         self._colortime = None
            # self._colortime = bs.gameTimer(200, colori, True)
            # def countdown(time):
            #     if self._powText.exists():
            #         self._powText.text = str(time)+'s'
            # interval = int(defaultPowerupInterval/1000) # 8
            # time2 = (interval-1)
            # time = 1
            # while(interval+3):           
            #     bs.gameTimer((time-1)*1000, 
            #         bs.Call(countdown, time2))
        ...
        # self._colorTimer = bs.Timer(
        #     500, bs.WeakCall(self._changeColor), repeat=True)
        ...
        if some.night:
            self.nodeLight = bs.newNode('light',
                                            ...
                                            attrs={
                                                ...
                                                'color': (random.uniform(0.5, 3), random.uniform(0.5, 3), random.uniform(0.5,3)),
                                                'radius': 0.3,
                                                'intensity': 0.33,
                                                'volumeIntensityScale': 1.0
                                            })
            self.node.connectAttr('position', self.nodeLight, 'position')
            #bs.animate(self.nodeLight, 'intensity', {0:0.2, 500: 0.4, 1000:0.2})
    ...
    def _changeColor(self):
        if self.nodeShield.exists():
            self.nodeShield.color = (random.random() * 2, random.random() * 2,
                                     random.random() * 2)
        ...
        if isinstance(m, PowerupAcceptMessage):
            ...
            if self.node.exists():
                if (m.immediate):
                    ...
                    if hasattr(self, 'nodeShield'):
                        self.nodeShield.delete()
                    if hasattr(self, '_colorTimer'):
