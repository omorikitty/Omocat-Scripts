...
class Icon(bsElimination.Icon):
    def updateForLives(self):
        ...
        if self._showLives:
            if lives > 0:
                self._livesText.text = u'x' + str(lives - 1)
            elif lives < 0:
    ...
    def _updateDamageStatus(self, damageStatus, color, glow=2):
        self._livesText.text = str(damageStatus) + '%'
        self._livesText.color = (color[0] * glow, color[1] * glow, color[2] * glow)
        self._livesText.shadow = 2
        scale = self._livesText.scale
        bs.animate(self._livesText, "scale", {
            0: self._livesText.scale,
            10: self._livesText.scale * 2,
            25: self._livesText.scale,
            30: self._livesText.scale * 2,
            35: self._livesText.scale
        })
        setattr(self._livesText, "scale", scale)
...
class PowBox(Bomb):
    ...
    def handleMessage(self, m):
        if isinstance(m, bs.PickedUpMessage):
            ...
            bs.PopupText(u'Fly!', self.node.position).autoRetain()
        ...
        if isinstance(m, bs.HitMessage):
            ...
            if self.hitPoints > 0:
                ...
                if damage/2000 > 0.0:
                    ...
                    # # print(stat)
                    # self.setScoreText(
                    #     str(int((self.multiplyer-1)*100))+"%", color=(0.5, 0.5, 1))
                    try:
                        for icon in self._player.gameData['icons']:
                            icon._updateDamageStatus(int(
                                (self.multiplyer-1)*100), self._player.color)
...
class SuperSmash(bs.TeamGameActivity):
    ...
    def onPlayerJoin(self, player):
        if self.hasBegun():
            ...
            bs.screenMessage(bs.Lstr(resource='playerDelayedJoinText', subs=[
                             ('${PLAYER}', player.getName(full=True))]), color=player.color)
            return
