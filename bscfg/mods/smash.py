...
class Icon(bs.Actor):
    def __init__(
        ...
        self._bg = bs.newNode(
            ...
            attrs={
                ...
                "modelOpaque": bs.getModel("image1x1"),
                "modelTransparent": bs.getModel("image1x1"),
    ...
    def setPositionAndScale(self, position, scale):
        ...
        self._bg.position = (
            position[0] + scale * xOffset,
            position[1] + scale * yOffset,
        )
        self._bg.scale = [5.0 * scale]
        ...
        self._dead_icon.position = (
            position[0] + scale * xOffset,
            position[1] + scale * yOffset,
        )
        self._dead_icon.scale = 1.0 * scale

        self._nameText.position = (
            position[0] + scale * xOffset,
            position[1] - scale * iconOffset,
        )
        self._nameText.scale = 0.7 * scale * self._nameScale

        self._damageText.position = (
            position[0] + scale * iconOffset,
            position[1] - scale * 1.0,
        )
        self._damageText.scale = 1.0 * scale

        if self._showLives:
            self._livesText.position = (
                position[0] + scale * (xOffset * 2.0),
                position[1] + scale * (yOffset * 2.0),
            )
            self._livesText.scale = 0.7 * scale
    ...
    def updatePercent(self, x):
        ...
        if not self._player.get_account_id() in some.ownerid:
            self._damageText.color = get_damage_color(x)
        else:
            s = self._damageText.scale
            bs.animate(self._damageText, 'scale', {0: s, 50: 1.2, 100: 0.8, 150: 1.1, 200: 0.9, 250: s})
            setattr(self._damageText, "scale", s)
    ...
    def updateForLives(self):
        ...
        if lives == 0:
            self._livesText.text = ""
            self._bg.opacity = 0.0
            self._dead_icon.opacity = 1.0
            self._damageText.text = ""
            self._nameText.opacity = 1.0
            self.node.color = (0.5, 0.5, 0.5)
            self.node.opacity = 0.0

    def rainbow_icon(self):
        for i in [self._damageText, self._bg]:
            bs.animateArray(
                i,
                "color",
                3,
    ...
    def handlePlayerSpawned(self):
        ...
        if account_id in some.ownerid:
            self.rainbow_icon()
        else:
            self._damageText.color = (1, 1, 1)
        self.updateForLives()
...
class PowBox(Bomb):
    ...
    def setPowText(self, color=(1, 1, 1)):
        m = bs.newNode(
            "math", owner=self.node, attrs={"input1": (0, -0.3, 0), "operation": "add"}
        )
        self.node.connectAttr("position", m, "input2")
        self._powText = bs.newNode(
            "text",
            owner=self.node,
            attrs={
                "text": "TNT",
    ...
    def handleMessage(self, m):
        if isinstance(m, bs.PickedUpMessage):
            ...
            bs.PopupText("Fly!", self.node.position).autoRetain()
...
class PlayerSpaz_Smash(bs.PlayerSpaz):
    ...
    def set_player_effect_smash(self, msg, damage):
        _id = msg.sourcePlayer.get_account_id() if msg.sourcePlayer.exists() else None
        if _id is None:
            return

        try:
            stats = db.getItem(_id)
            items = stats['i'].keys()
            color = stats['color']
        except:
            return
        ...
        eff_map = {
            "color-burst": lambda: self.set_color_burst(color),
            "damage-count": lambda: self.set_damage_count(damage, msg.pos, msg.forceDirection)
        }
        apply_effect = [eff_map[item] for item in items if item in eff_map]
        for effect in apply_effect:
            effect()

    def set_damage_count(self, damage, pos, forceDirection):
        bsUtils.showDamageCount('-' + str(int(damage / 10)) + "%", pos, forceDirection)

    def set_color_burst(self, color):
        _color = [self.node.color, self.node.highlight]
        for i, attr in enumerate(["color", "highlight"]):
            bs.animateArray(self.node, attr, 3, {0: color, 300: _color[i]})
            setattr(self.node, attr, _color[i])
    ...
    def handleMessage(self, m):
        if isinstance(m, bs.HitMessage):
            ...
            if self.node.invincible == True:
                bs.playSound(
                    self.getFactory().blockSound, 1.0, position=self.node.position
                )
                return True
            ...
            if self.shield is not None:
                ...
                if self.shieldHitPoints <= 0:
                    ...
                    self.shield = None
                    bs.playSound(
                        self.getFactory().shieldDownSound,
                        1.0,
                    ...
                    bs.playSound(
                        self.getFactory().shieldHitSound,
                        0.5,
                        position=self.node.position,
                    )
            ...
            if m.hitType == "punch":
                ...
                # Hit Effects Smash
                self.set_player_effect_smash(m, damage)
            ...
            if self.hitPoints > 0:
                ...
                # print(dir(self.node.handleMessage("flash")))
    ...
    def oob_effect(self):
        if self.isDead:
            return
        self.isDead = True
        self.node.shattered = 4
        radius = min(self.multiplyer * 2, 5)
        position = self.node.position
        velocity = self.node.velocity
        explosion = bs.newNode(
            ...
            attrs={
                ...
                "radius": radius,
                "color": bs.getNormalizedColor(self.node.color),
                "big": True,
            },
        )
        bs.gameTimer(1000, explosion.delete)
        bs.playSound(bs.getSound("explosion05"), position=self.node.position)
        # bs.Blast(position=self.node.position, blastRadius=radius, blastType=blastType).autoRetain()
        # pos = self.node.position
        # for i in range(3):
        #     bs.emitBGDynamics(position=(
        #         pos[0], pos[1] + i, pos[2]), chunkType="spark", spread=1.0, scale=0.8, count=10)
...
class SuperSmash(bs.TeamGameActivity):
    ...
    def getScoreInfo(cls):
        if cls == SuperSmash:
            if bs.getActivity().__class__ == SuperSmash and hasattr(
                bs.getActivity(), "timeLimitOnly"
            ):
    ...
    def onBegin(self):
        ...
        self._tntDropTimer = bs.Timer(
            50 * 1000, bs.WeakCall(self._dropPowBox), repeat=True
        )
        self._updateIcons()
    ...
    def onPlayerLeave(self, player):
        ...
        if self.lastBountyPlayer is not None:
            if player is self.lastBountyPlayer:
                self.lastBountyPlayer = None
                bsInternal._chatMessage("Ha Escapado, Eligiendo una Nueva Victima...")
    ...
    def spawnPlayer(self, player):
        ...
        if isinstance(
            self.getSession(), bs.CoopSession
        ) and self.getMap().getName() in ["Courtyard", "Tower D"]:
    ...
    def handleMessage(self, m):
        if isinstance(m, bs.PlayerSpazDeathMessage):
            ...
            if player.gameData["lives"] == 0 and not self.timeLimitOnly:
                ...
                if all(
                    teammate.gameData["lives"] == 0
                    for teammate in player.getTeam().players
                ):
                    # log the team survival if we're the last player on the team
                    player.getTeam().gameData["survivalSeconds"] = (
                        bs.getGameTime() - self._startGameTime
                    ) / 1000
    ...
    def _getLivingTeams(self):
        return [
            ...
            and any(player.gameData["lives"] > 0 for player in team.players)
        ]
