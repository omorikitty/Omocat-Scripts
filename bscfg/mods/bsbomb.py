...
class NewBombFactory(BombFactory):
    def __init__(self):
        ...
        if some.custom_tnt:
            self.tntModel = bs.getModel('bomb')
        self.curseBombModel = bs.getModel('bomb')

        self.forceTex = bs.getTexture('egg2')
        if some.custom_tnt:
            self.tntTex = bs.getTexture('powerupCurse')
...
def __init__(self,
    ...
    if self.blastType == "ice":
        ...
        explosion.color = (random.uniform(0.5, 1), random.uniform(
            0.5, 1), random.uniform(0.5, 1))
